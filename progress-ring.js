...
(function ($) {
    $.fn.loadingRing = function () {
        ...
        $(this).each(function () {
            var $target = $(this);
            var color = $target.data('color'); // 棰滆壊
            var precent = parseInt($target.data('precent'), 10); // 鐧惧垎姣�
            var duration = parseFloat($target.data('duration'), 10) * 1000; // 鎸佺画鏃堕棿
            ...
            var x = $target.find('.progress-cover').height(); // 瑙﹀彂 Layout
